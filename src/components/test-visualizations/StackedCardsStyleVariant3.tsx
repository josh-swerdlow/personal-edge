...
// Variant 3: Gradient - Gradient background
export default function StackedCardsStyleVariant3({ deck }: StackedCardsStyleProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="text-center mb-4">
        <h3 className="text-lg font-bold text-gray-800">Variant 3: Blue/Purple</h3>
        ...
        <div className="relative" onDoubleClick={handleDoubleClick}>
          ...
          <div className="flex gap-3 overflow-x-auto md:overflow-hidden scrollbar-hide">
            {currentSection.cards.map((card) => (
              <div
                key={card.id}
                className="flex-shrink-0 bg-gradient-to-br from-blue-50 to-purple-50 border-2 border-blue-200 rounded-2xl p-4 shadow-lg transition-all duration-300"
                style={{
            ...
            {sectionCards
              ...
              .map((card, stackIndex) => {
                ...
                return (
                  <div
                    key={card.id}
                    className="absolute bg-gradient-to-br from-blue-50 to-purple-50 border-2 border-blue-200 rounded-2xl p-4 shadow-lg cursor-pointer transition-all duration-300"
                    style={{
                      left: `${stackOffset}px`,
                      top: `${stackOffset}px`,
                      ...
                      zIndex,
                      width: 'clamp(280px, 80vw, 400px)',
                      minHeight: 'clamp(168px, 60vw, 240px)',
                      minWidth: '280px',
                      opacity,
                    }}
                  ...
                  >
                    ...
                    {isTopCard && (
                      <>
                        {/* Left Zone - half size */}
                        <div
                          className={`absolute inset-y-0 left-0 w-1/12 bg-gray-400/10 transition-opacity duration-200 ${
                            hoverZone === 'left' ? 'opacity-100' : 'opacity-0'
                          }`}
                        />
                        {/* Right Zone - half size */}
                        <div
                          className={`absolute inset-y-0 right-0 w-1/12  bg-gray-400/10 transition-opacity duration-200 ${
                            hoverZone === 'right' ? 'opacity-100' : 'opacity-0'
                          }`}
                        />
                        {/* Top Zone */}
                        <div
                          className={`absolute inset-x-0 top-0 h-1/6 bg-gray-400/10 transition-opacity duration-200 ${
                            hoverZone === 'top' ? 'opacity-100' : 'opacity-0'
                          }`}
                        />
                        {/* Bottom Zone */}
                        <div
                          className={`absolute inset-x-0 bottom-0 h-1/6 bg-gray-400/10 transition-opacity duration-200 ${

...
export interface StackedCardsStyleProps {
  deck: Deck;
  gradientFrom: string; // e.g., "blue-50"
  gradientTo: string; // e.g., "purple-50"
  borderColor: string; // e.g., "blue-200"
  variantName: string; // e.g., "Blue/Purple"
  // Text extractors - functions to get text from card
  getTitle?: (card: Card, sectionTitle: string) => string;
  getContent?: (card: Card) => string;
  getTags?: (card: Card) => string[];
  // Display feature toggles
  showPriorityBadge?: boolean; // Show ! for high priority
  showHelpfulnessScore?: boolean; // Show +X for upvotes
  showStar?: boolean; // Show ★ for priority
}
  ...
  return (
    <div className="flex flex-col h-full min-h-0 relative z-10">
      <div className="flex-1 overflow-y-auto pr-1 space-y-2">
        {title && <h5 className="text-sm font-semibold text-gray-800">{title}</h5>}
        {content && <p className="text-base text-gray-700 leading-relaxed whitespace-pre-line">{content}</p>}
      </div>
      <div className="mt-3 pt-3 border-t-2 border-gray-300">
        <div className="flex items-center justify-between pt-2">
          ...
          <div className="flex items-center gap-2 text-xs font-semibold text-gray-600">
            {showPriorityBadge && card.priority && (
              <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-red-100 text-red-600">
                !
              </span>
            )}
            {showHelpfulnessScore && card.helpfulnessScore > 0 && (
              <span className="inline-flex items-center justify-center rounded-full bg-emerald-100 text-emerald-700 px-2 py-0.5">
                +{card.helpfulnessScore}
              </span>
            )}
            {showStar && card.priority && (
...
export default function StackedCardsStyleGeneric({
  deck,
  gradientFrom,
  gradientTo,
  borderColor,
  ...
  // Build dynamic class names for gradient and border
  // Using conditional to ensure Tailwind can detect all class names at build time
  let gradientClass = 'bg-gradient-to-br';
  if (gradientFrom === 'blue-50' && gradientTo === 'purple-50') {
    gradientClass = 'bg-gradient-to-br from-blue-50 to-purple-50';
  } else if (gradientFrom === 'pink-50' && gradientTo === 'orange-50') {
    gradientClass = 'bg-gradient-to-br from-pink-50 to-orange-50';
  } else if (gradientFrom === 'green-50' && gradientTo === 'teal-50') {
    gradientClass = 'bg-gradient-to-br from-green-50 to-teal-50';
  }

  let borderClass = 'border-2';
  if (borderColor === 'blue-200') {
    borderClass = 'border-2 border-blue-200';
  } else if (borderColor === 'pink-200') {
    borderClass = 'border-2 border-pink-200';
  } else if (borderColor === 'green-200') {
  ...
  return (
    <div className="space-y-4">
      <div className="text-center mb-4">
        <h3 className="text-lg font-bold text-gray-800">{variantName}</h3>
        ...
        <div className="relative" onDoubleClick={handleDoubleClick}>
          ...
          <div className="flex gap-3 overflow-x-auto md:overflow-hidden scrollbar-hide">
            {currentSection.cards.map((card) => (
              <div
                key={card.id}
                className={`flex-shrink-0 ${gradientClass} ${borderClass} rounded-2xl p-4 shadow-lg transition-all duration-300 flex flex-col min-h-0`}
                style={{
              ...
              >
                <CardContent
                  sectionTitle={currentCard.sectionTitle}
                  card={card}
                  getTitle={getTitle}
                  getContent={getContent}
                  getTags={getTags}
                  showPriorityBadge={showPriorityBadge}
                  showHelpfulnessScore={showHelpfulnessScore}
                  showStar={showStar}
                />
              </div>
            ))}
            ...
            {sectionCards
              ...
              .map((card, stackIndex) => {
                ...
                return (
                  ...
                  <div
                    key={card.id}
                    className={`absolute ${gradientClass} ${borderClass} rounded-2xl p-4 shadow-lg cursor-pointer transition-all duration-300 flex flex-col min-h-0`}
                    style={{
                      left: `${stackOffset}px`,
                      top: `${stackOffset}px`,
                  ...
                  >
                    ...
                    {isTopCard && (
                      <>
                        ...
                        {/* Top Zone */}
                        <div
                          className={`absolute inset-x-0 top-0 h-1/4 bg-gray-400/10 transition-opacity duration-200 ${
                            hoverZone === 'top' ? 'opacity-100' : 'opacity-0'
                          }`}
                        />
                        {/* Bottom Zone */}
                        <div
                          className={`absolute inset-x-0 bottom-0 h-1/4 bg-gray-400/10 transition-opacity duration-200 ${
